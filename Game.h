...
public:
	...
	void moveCard(string src, string dest, int numb)
	{
		List::iterator itr;
	...
	bool enterCommand()
	{
		...
		else if (x == 'm')
		{
			if(c->validMove())
			{
				moveCard(c->getSource(), c->getDestination(), c->getNumber());
				cStack.push(c);
				return true;
			}
			else
